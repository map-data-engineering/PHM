...
std <- raw %>%
  ...
  mutate(addo_uid = sprintf("TZ-ADDO-%05d", row_number())) %>%
  # Drop exact re-surveys: same name/region/district/ward/coords, keeping
  # the first-entered record so surviving uids never shift on re-run.
  distinct(
    addo_name_raw, region_raw, district_raw, ward_raw,
    lat_tablet, lon_tablet, lat_hand, lon_hand,
    .keep_all = TRUE
  ) %>%
  mutate(
