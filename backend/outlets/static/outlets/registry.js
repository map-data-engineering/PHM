(function () {
  ...
  apiGet("boundaries/region/").then(gj => {
  ...
  els.fRegion.addEventListener("input", () => {
    ...
    if (els.fRegion.value) {
      apiGet("boundaries/district/", { region: els.fRegion.value }).then(gj => {
        for (const f of gj.features.sort((a, b) => a.properties.name.localeCompare(b.properties.name))) {
  ...
  els.fDistrict.addEventListener("input", () => {
    ...
    if (els.fDistrict.value) {
      apiGet("boundaries/ward/", { district: els.fDistrict.value }).then(gj => {
        for (const f of gj.features.sort((a, b) => a.properties.name.localeCompare(b.properties.name))) {
  ...
  async function applyFilters() {
    ...
    try {
      [rows, geo] = await Promise.all([
        apiGet("outlets/points/", pointParams()),
        apiGet(`boundaries/${currentView}/`, boundaryParams(currentView)),
      ]);
    } catch (err) {
